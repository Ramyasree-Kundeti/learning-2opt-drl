...
import pandas as pd
import mp_utils_segment_shift as mp_utils
...
print(df)
avg_best_distances = np.mean(result)
print("avg_best_distances ",avg_best_distances)
print("test_data.opt len",len(test_data.opt))
print("test_data.opt mean",np.mean(test_data.opt))
gap = (avg_best_distances / np.mean(test_data.opt)) * 100
print('Best Cost: {:.5f} Opt Cost: {:.5f} Gap: {:.2f} %'.format(
    avg_best_distances/10000, np.mean(test_data.opt), gap))
