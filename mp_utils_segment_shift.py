...
def list_indices(n_points):
    ...
    for i in range(n_points):
        for j in range(i + 2, n_points - 1):
            for k in range(j + 2, n_points - 2 + (i > 0)):
...
def swapEdgesSegmentShiftOPT(tour, i, j, k):
    """
        ...
        for i in range(len(best_tour)):
            for j in range(i + 2, len(best_tour) - 1):
                for k in range(j + 2, len(best_tour) - 2 + (i > 0)):
                    ...
                    new_tour = swapEdgesSegmentShiftOPT(tour.copy(), i, j, k)
...
def heuristic_ss_fi_restart(positions, steps):
    # tracking improvemnt in tour
    improved = True
    tour = [x for x in range(len(positions))]
    best_tour = tour
...
def heuristic_ss_bi_restart(positions, steps):
    # tracking improvemnt in tour
    # print("heuristic_3opt_bi is called")
    improved = True
    tour = [x for x in range(len(positions))]
